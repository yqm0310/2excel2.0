    ...
    try:
        ...
        if os.path.splitext(file1[i])[1] == ".xlsx":
            for r in range(1, maxrow):
                ...
                if sheet1_cell_value.value not in cols2:
...
#处理完成的结果文件路径与目标文件一致  目标文件名后面加_result.xlsx
        srcPath = os.path.splitext(file1[i])[0]
        #print(srcPath)
        absPath = srcPath + baseName
        wb_result.save(absPath)
        #print(absPath)
